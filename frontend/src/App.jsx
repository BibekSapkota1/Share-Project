...
//             data.latest_signal.type === 'OVERBOUGHT' ? 'buy' : 'sell');
//         }
//       } else {
//         showNotificationMessage(data.error, 'error');
//       }
...
import React, { useState, useEffect } from 'react';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ReferenceLine } from 'recharts';
import { TrendingUp, TrendingDown, Activity, Settings, Bell, AlertCircle, Database, Search, BarChart3, History, DollarSign } from 'lucide-react';
...
function App() {
  ...
  const fetchSymbols = async () => {
    try {
      ...
      console.error('Error fetching symbols:', error);
      showNotificationMessage('Cannot connect to backend', 'error');
    }
  };
  ...
  const filteredScannerData = scannerData.filter(item => {
    ...
    return item.signal === filterSignal;
  });
