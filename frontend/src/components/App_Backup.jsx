...
import TradeCycles from './components/TradeCycles';

import { verifyTokenAPI, fetchSymbolsAPI, fetchScannerAPI, fetchCyclesAPI, executeTradeAPI } from './utils/api';

// ─────────────────────────────────────────────────────────────────────────────
// App  – root component
// Responsibilities: auth flow, active-tab state, global notification, top nav.
// Everything else is delegated to Scanner / Analysis / TradeCycles.
// ─────────────────────────────────────────────────────────────────────────────
export default function App() {
  ...
  // ── whenever we become authenticated, load initial data ───────────────────
  useEffect(() => {
    if (isAuthenticated) {
      fetchSymbols();
      fetchScanner();
  ...
  // ── load cycles when the user switches to that tab ────────────────────────
  useEffect(() => {
    if (activeTab === 'cycles' && isAuthenticated) fetchCycles();
  }, [activeTab]);

  // ── data fetchers ─────────────────────────────────────────────────────────
  const fetchSymbols = async () => {
    try {
      const res  = await fetchSymbolsAPI();
  ...
  const fetchScanner = async () => {
    setScannerLoading(true);
    try {
      const res  = await fetchScannerAPI();
      const data = await res.json();
      if (res.ok) {
        setScannerData(data.symbols);
        setScannerSummary(data.summary);
    ...
    } catch { showNotificationMessage('Error fetching scanner data', 'error'); }
  ...
  const fetchCycles = async (symbol = null) => {
    setCyclesLoading(true);
    try {
      const res  = await fetchCyclesAPI(symbol);
      const data = await res.json();
      if (res.ok) {
        setAllCycles(data.cycles);
      } else {
    ...
    } catch { showNotificationMessage('Error fetching cycles', 'error'); }
    finally  { setCyclesLoading(false); }
  };

  const executeTrade = async (symbol, action, date, price, rsi) => {
    try {
      const res  = await executeTradeAPI(symbol, action, date, price, rsi);
      const data = await res.json();
      if (res.ok) {
        showNotificationMessage(data.message, 'success');
        fetchScanner();                                   // refresh scanner
        if (activeTab === 'cycles') fetchCycles();        // refresh cycles if visible
      } else {
        if (res.status === 401) handleLogout();
        showNotificationMessage(data.error, 'error');
      }
    } catch { showNotificationMessage('Error executing trade', 'error'); }
  };
  ...
  if (!isAuthenticated) {
    return (
      <Login
        onAuthSuccess={(u) => { setUser(u); setIsAuthenticated(true); }}
    ...
    <div className="app">
      ...
      <header className="header">
        <div className="header-content">
          ...
          <nav className="nav">
            <NavBtn active={activeTab === 'scanner'}  onClick={() => setActiveTab('scanner')}><Search size={18} /> Scanner</NavBtn>
            <NavBtn active={activeTab === 'analysis'} onClick={() => setActiveTab('analysis')}><BarChart3 size={18} /> Analysis</NavBtn>
            <NavBtn active={activeTab === 'cycles'}   onClick={() => setActiveTab('cycles')}><History size={18} /> Trade Cycles</NavBtn>
          </nav>

          <div className="user-section">
            <div className="user-info"><User size={18} /><span>{user?.email}</span></div>
            <button className="logout-btn" onClick={handleLogout}><LogOut size={18} /> Logout</button>
          </div>
        </div>
      </header>

      {/* ── Page Content (tab-switched) ── */}
      <main className="main-content">
        {activeTab === 'scanner' && (
          <Scanner
            ...
            onViewDetails={(sym) => { setSelectedSymbol(sym); setActiveTab('analysis'); }}
          />
        )}
      ...
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'SF Mono','Consolas','Monaco',monospace; background: #0a0e1a; color: #e2e8f0; }
        ...
        .header { position: relative; z-index: 10; background: rgba(15,20,25,0.8); backdrop-filter: blur(10px); border-bottom: 1px solid #1e293b; }
        .header-content { max-width: 1600px; margin: 0 auto; padding: 1.5rem 2rem; display: flex; justify-content: space-between; align-items: center; }
        .logo { display: flex; align-items: center; gap: 0.75rem; }
        .logo svg { color: #10b981; filter: drop-shadow(0 0 8px rgba(16,185,129,0.5)); }
        .logo h1 { font-size: 1.5rem; font-weight: 800; letter-spacing: -0.02em; color: #f1f5f9; }
        .logo h1 span { color: #10b981; }
        .nav { display: flex; gap: 0.5rem; }
        .nav button { background: none; border: none; color: #94a3b8; padding: 0.75rem 1.5rem; cursor: pointer; font-size: 0.95rem; font-weight: 600; font-family: inherit; transition: all 0.3s ease; border-radius: 8px; display: flex; align-items: center; gap: 0.5rem; }
        .nav button:hover { color: #10b981; background: rgba(16,185,129,0.1); }
        .nav button.active { color: #10b981; background: rgba(16,185,129,0.15); }
        .user-section { display: flex; align-items: center; gap: 1rem; }
        .user-info { display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; background: rgba(30,41,59,0.5); border-radius: 8px; color: #94a3b8; font-size: 0.9rem; }
        .logout-btn { display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); color: #ef4444; border-radius: 8px; cursor: pointer; font-weight: 600; font-family: inherit; transition: all 0.3s ease; }
        .logout-btn:hover { background: rgba(239,68,68,0.2); transform: translateY(-2px); }
        .main-content { position: relative; z-index: 1; max-width: 1600px; margin: 0 auto; padding: 2rem; }
        .notification { position: fixed; top: 100px; right: 2rem; display: flex; align-items: center; gap: 0.75rem; padding: 1rem 1.5rem; border-radius: 12px; font-weight: 600; z-index: 1000; transform: translateX(400px); transition: transform 0.4s ease; box-shadow: 0 10px 40px rgba(0,0,0,0.3); }
        ...
        /* scanner shared colours used by child filter-btn */
        .filter-btn { background: rgba(30,41,59,0.5); border: 1px solid #1e293b; color: #94a3b8; padding: 0.75rem 1.5rem; border-radius: 8px; font-weight: 600; cursor: pointer; font-family: inherit; transition: all 0.3s ease; }
        ...
        .filter-btn.buy.active { background: rgba(16,185,129,0.15); border-color: #10b981; color: #10b981; }
        .filter-btn.sell.active { background: rgba(239,68,68,0.15); border-color: #ef4444; color: #ef4444; }
        .filter-btn.neutral.active { background: rgba(100,116,139,0.15); border-color: #64748b; color: #94a3b8; }
        .filter-btn.holdings.active { background: rgba(59,130,246,0.15); border-color: #3b82f6; color: #3b82f6; }
        /* summary cards – shared */
        .summary-card { background: rgba(15,20,25,0.6); border: 1px solid #1e293b; border-radius: 12px; padding: 1.5rem; display: flex; align-items: center; gap: 1rem; transition: all 0.3s ease; }
        .summary-card:hover { transform: translateY(-2px); border-color: #10b981; }
        .summary-card.buy { border-color: rgba(16,185,129,0.3); background: rgba(16,185,129,0.05); }
        .summary-card.sell { border-color: rgba(239,68,68,0.3); background: rgba(239,68,68,0.05); }
        .summary-card.neutral { border-color: rgba(100,116,139,0.3); background: rgba(100,116,139,0.05); }
        .summary-card.active { border-color: rgba(59,130,246,0.3); background: rgba(59,130,246,0.05); }
        .summary-icon { font-size: 2rem; }
        .summary-value { font-size: 2rem; font-weight: 800; color: #f1f5f9; }
        .summary-label { font-size: 0.85rem; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.05em; }
        /* cycle-row – used inside scanner cards */
        .cycle-row { display: flex; justify-content: space-between; font-size: 0.85rem; }
        .cycle-row span { color: #94a3b8; }
        .cycle-row strong { color: #f1f5f9; }
        .cycle-row .profit { color: #10b981; }
        .cycle-row .loss { color: #ef4444; }
      `}</style>
    </div>
  );
}

// ── tiny nav button helper ──────────────────────────────────────────────────
